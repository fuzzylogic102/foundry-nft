// SPDX-License-Identifier: MIT
pragma solidity 0.8.20;

import {MoodNFT} from "../../src/MoodNFT.sol";
import {Test} from "forge-std/Test.sol";
import "forge-std/console.sol";

contract MoodtNFTTest is Test {
    MoodNFT moodNft;
    address USER = makeAddr("user");
    string public constant SAD_SVG_URI = "data:image/svg+xml;base64,";
    string public constant HAPPY_SVG_URI = "data:image/svg+xml;base64,";
    function setUp() public {
        moodNft = new MoodNFT(SAD_SVG_URI, HAPPY_SVG_URI);
    }

    function testViewTokenURI() public {
        vm.prank(USER); //without adding this it will throw an error since open zeppelin added in their ERC contract that you cannot send NFTs to contracts that can't do anyhting
        //with them so they aren't lost. This way we will be sending them to an address and not a contract 
        moodNft.mintNft();
        console.log(moodNft.tokenURI(0)); 
    }
}
